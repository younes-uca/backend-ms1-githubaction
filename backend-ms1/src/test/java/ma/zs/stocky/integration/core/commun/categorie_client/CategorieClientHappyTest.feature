Feature: CategorieClient

  Background:
    #* call read('karate-config.js')
    #* call read('db_cleaner.js')
    * url categorieClientUrl
    * header Content-Type = 'application/json'

    * def postBody = read('CategorieClientSave.json')
    * def putBody = read('CategorieClientUpdate.json')
    * def objectSchema = read('CategorieClientSchema.json')
